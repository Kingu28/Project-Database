...
if(isset($_POST['tambah'])){
	...
	if( !empty($noport) && !empty($ruaskabel) && !empty($panjangkabel) && !empty($valuekabel) && !empty($jumlah) && !empty($loss) && !empty($tujuan) && !empty($titikbend) && !empty($lokasibend) && !empty($status) && !empty($tipe) && !empty($merk) ){
		$query = "INSERT INTO datakabel (no_port,ruaskbl,panjangkbl,valuekbl,jumlah_port,total_loss,tempat,titik_bending,lokasi_bending, statuscore, tipekbl, merkkbl) 
					VALUES ('$noport','$ruaskabel','$panjangkabel','$valuekabel','$jumlah','$loss','$tujuan','$titikbend','$lokasibend','$status','$tipe','$merk') ";
		$daftar = data($query);
		if($daftar){
			header('Location: index.php');
		}else{
	...
	<!-- <div class="pendaftaran"> -->
		<div class="form-abu border">
			...
			<form action="" method="post">
				...
				<input type="number" class="full" name="panjang">
				...
				<input type="number" class="full" name="jumlah">
				<label for="">Total Loss (Kumulatif)</label>
				<input type="number" class="full" name="loss"> 
				<label for="">Titik Bending</label>
				<input type="number" select name="titik" class="full">
